...
from varainterface import Account, constants, Datalog, Launch, SubEvent, Subscriber
...
def datalog(input_string: sys.stdin, remote_ws: str, s: str) -> None:
    """
    Save string into account's datalog using pipeline:  <echo "blah" | vara_interface io write datalog (params)>
...
def launch(command: sys.stdin, remote_ws: str, s: str, r: str) -> None:
    ...
    <echo "Qmc5gCcjYypU7y28oCALwfSvxCBskLuPKWpK4qpterKC7z" | vara_interface io write launch (params)>
    If nothing passed, waits for a string in a new line.
    """
    account: Account = Account(remote_ws=remote_ws, seed=s)
